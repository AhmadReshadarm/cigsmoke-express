...
export class BasketController {
  ...
  async getBaskets(req: Request, resp: Response) {
    ...
    const baskets = await this.basketService.getBaskets(req.query, req.headers.authorization!);
  ...
  async getBasket(req: Request, resp: Response) {
    const { id } = req.params;
    const basket = await this.basketService.getBasket(id, req.headers.authorization!);
